...
class AsyncImagine:
    ...
    async def upscale(self, content: bytes, asbase64: bool = False) -> bytes:
        """Upscale"""
        try:
            response = await self._request(
                method="POST",
    ...
    async def interrogator(self, content: bytes) -> str:
        """Generate Prompt"""
        try:
            response = await self._request(
                method="POST",
    ...
    async def codeformer(self, content: bytes, asbase64: bool = False) -> bytes:
        """Face Fixed"""
        try:
            response = await self._request(
                method="POST",
